...
<body>
    <nav class="navbar">
        <div class="nav-container">
            <a href="/" class="nav-brand">🗜️ Data Compression</a>
            <ul class="nav-links">
                <li><a href="/" class="{home_active}">🏠 Home</a></li>
                <li><a href="/images" class="{images_active}">🖼️ Images</a></li>
                <li><a href="/videos" class="{videos_active}">🎥 Videos</a></li>
                <li><a href="/documents" class="{docs_active}">📄 Documents</a></li>
                <li><a href="/text" class="{text_active}">📝 Text</a></li>
            </ul>
        </div>
    </nav>
            ...
            if (data.filename) {
                html += `
                    <div class="file-info">
                        ...
                        ${data.uploaded_file_size ? `
                        <div class="file-info-item">
                            <span>Uploaded File Size:</span>
                            <strong>${formatBytes(data.uploaded_file_size)}</strong>
                        </div>
                        <div class="file-info-item" style="background: #fff3cd; padding: 10px; border-radius: 5px; margin-top: 10px;">
                            <span style="color: #856404; font-size: 0.9em;">
                                ℹ️ Note: Algorithms compress the <strong>raw pixel data</strong> (${formatBytes(data.results[0].original_size)}), not the PNG file.
                                ${data.results.some(r => r.compressed_size < data.uploaded_file_size) ? 
                                    '<br>✅ Some algorithms achieved better compression than the original PNG!' : ''}
                            </span>
                        </div>
                        ` : ''}
                ...
                html += `
                    <div class="result-card">
                        ...
                        <div style="margin-top: 15px; text-align: center; display: flex; gap: 10px; justify-content: center; flex-wrap: wrap;">
                            ${result.binary_file ? `
                            <a href="/download/${result.binary_file}" class="download-btn" style="background: linear-gradient(135deg, #f093fb 0%, #f5576c 100%);">
                                💾 Download .BIN (${formatBytes(result.compressed_size)})
                            </a>
                            ` : ''}
                            ${result.download_file ? `
                            <a href="/download/${result.download_file}" class="download-btn" style="background: linear-gradient(135deg, #11998e 0%, #38ef7d 100%);">
                ...
                html += `
                    <div class="summary">
                        ...
                        ${data.summary.best_compression ? 
                            `<p>📦 Best Compression: <strong>${data.summary.best_compression.name}</strong> (${data.summary.best_compression.saving.toFixed(2)}% saved)</p>` : ''}
                        ${data.summary.fastest_comp ? 
                            `<p>⚡ Fastest Compression: <strong>${data.summary.fastest_comp.name}</strong> (${data.summary.fastest_comp.time.toFixed(6)}s)</p>` : ''}
                    </div>
                `;
            }
    ...
    <div class="content">
        <form id="uploadForm" enctype="multipart/form-data">
            ...
            <div style="text-align: center;">
                <button type="submit" class="btn">🗜️ Compress Image</button>
                <button type="button" class="btn btn-secondary" onclick="resetForm()">🗑️ Clear</button>
            </div>
        </form>
...
<div class="container">
    ...
    <div class="content">
        <form id="uploadForm" enctype="multipart/form-data">
            ...
            <div style="text-align: center;">
                <button type="submit" class="btn">🗜️ Compress Video</button>
                <button type="button" class="btn btn-secondary" onclick="resetForm()">🗑️ Clear</button>
            </div>
        </form>
...
<div class="container">
    ...
    <div class="content">
        <form id="uploadForm" enctype="multipart/form-data">
            ...
            <div style="text-align: center;">
                <button type="submit" class="btn">🗜️ Compress Document</button>
                <button type="button" class="btn btn-secondary" onclick="resetForm()">🗑️ Clear</button>
            </div>
        </form>
...
• Repetitive: AAAABBBBCCCCDDDD (repeat many times)
• Natural: The quick brown fox jumps over the lazy dog...
• Pattern: Lorem ipsum dolor sit amet..."></textarea>
            </div>
            ...
            <div style="text-align: center;">
                <button type="submit" class="btn">🗜️ Compress Text</button>
                <button type="button" class="btn btn-secondary" onclick="loadSample()">📋 Load Sample</button>
                <button type="button" class="btn btn-secondary" onclick="resetForm()">🗑️ Clear</button>
            </div>
        </form>
...
def compress_image():
    ...
    try:
        ...
        uploaded_file_size = os.path.getsize(filepath)
...
if __name__ == '__main__':
    print("\n" + "="*60)
    print("  🌐 Data Compression Project - Multi-Page Interface")
    print("="*60)
    print("\n📱 Starting web server...")
    print("🌍 Open your browser and go to: http://localhost:8080")
    print("\n📂 Available pages:")
    print("   • http://localhost:8080/images - Image Compression")
    print("   • http://localhost:8080/videos - Video Compression")
    print("   • http://localhost:8080/documents - Document Compression")
    print("   • http://localhost:8080/text - Text Compression")
